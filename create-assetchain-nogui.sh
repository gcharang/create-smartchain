...
$srcdir/$launch -pubkey=$pubkey1 &
echo "started the first daemon in a new terminal"

while [ ! -f "/home/$USER/.komodo/$name/$name.conf" ]; do
  sleep 2
...
$srcdir/$launch -pubkey=$pubkey2 -datadir=$datadir/$name -addnode=localhost &
echo "started the second daemon in a new terminal"

until $srcdir/komodo-cli -ac_name=$name getinfo &>/dev/null; do
...
until $srcdir/komodo-cli -ac_name=$name -datadir=$datadir/$name getinfo &>/dev/null; do
...
if [ $# -eq 1 ]; then
  if [ "$1" = "explorer" ]; then
    ./install-explorer-nogui.sh force
  fi
elif [ $# -eq 0 ]; then
  echo "The explorer can be installed by executing the script: ./install-explorer-nogui.sh"
else
